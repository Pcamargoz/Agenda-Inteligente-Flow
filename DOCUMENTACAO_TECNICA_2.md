...
Templates agrupam itens (treinamentos e tarefas). O hub permite configurar cada item em sequência, definir horários e marcar items como “pulado” ou “aguardando agendar”.
...
| Cronograma | `rascunho`, `aguardando-cliente`, `confirmado` |
| Registro | `em-andamento`, `pendente`, `em-atendimento`, `atendido`, `concluido` |
| Template | `ativo` (implícito) com items `treinamento` e `tarefa` |
| OS | `rascunho`, `enviada`, `assinada` |
...
- Próximas recomendações: adicionar `docs/ACCEPTANCE_CHECKLISTS.md`, `docs/email_templates.md`, e um `docs/09_endpoints_por_fluxo.md` ou `openapi.yaml` para formalizar contratos de API.
